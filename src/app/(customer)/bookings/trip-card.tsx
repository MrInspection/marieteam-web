...
import { Ship, ArrowRight, Anchor, Clock, Waves } from "lucide-react"
...
export function TripCard({ crossing, isSelected, onSelect }: TripCardProps) {
    ...
    if(crossing.delayMinutes === undefined) return;
                ...
                <div className="flex flex-col space-y-4 w-full rounded-2xl p-4">
                    <div className="flex items-center justify-between w-full pb-2.5">
                        <div className="text-center">
                            ...
                            <div className="font-semibold text-lg">{crossing.route.departure}</div>
                        </div>
                        ...
                        <div className="text-center">
                            ...
                            <div className="font-semibold text-lg">{crossing.route.arrival}</div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t-2">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className={cn((crossing.delayMinutes > 0 ? "line-through text-muted-foreground" : ""), "text-sm font-medium")}>
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <div className="flex items-center space-x-1 text-red-700">
                                                ...
                                                <span className="text-sm">{crossing.delayMinutes} min late</span>
                                            </div>
                                        </TooltipTrigger>
                                        <TooltipContent>
                                            <p>Expected delay: {crossing.delayMinutes} minutes</p>
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            )}
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger>
                                        <div className={`flex items-center space-x-1 ${getSeaConditionInfo(crossing.seaCondition).color}`}>
                                            <Waves className="w-4 h-4" />
                                            <span className="text-sm">{getSeaConditionInfo(crossing.seaCondition).label}</span>
                                        </div>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <p>Sea condition: {getSeaConditionInfo(crossing.seaCondition).label}</p>
                                    </TooltipContent>
                                </Tooltip>
                            </TooltipProvider>
                        </div>
